    ...
    <section id="about" className="py-24 bg-gradient-hero relative">
      ...
      <div className="container mx-auto px-4 relative z-10">
        <div className="max-w-6xl mx-auto">
          ...
          <div 
            ref={bioRef}
            className={`bg-gradient-card rounded-2xl p-8 md:p-12 card-shadow border border-border mb-16 gradient-border reveal ${bioVisible ? 'visible' : ''}`}
          >
            ...
            <p className="text-muted-foreground leading-relaxed mb-6">
              Currently, I am a key contributor to <span className="text-foreground font-medium">Petrofac's Lakehouse Migration</span> initiative, transforming legacy data pipelines into modern, scalable Azure + Databricks architecture with robust governance frameworks. I am also preparing for the <span className="text-foreground font-medium">Certified Data Management Professional (CDMP)</span> certification.
            </p>
          ...
          <h3 
            ref={skillsRef}
            className={`font-display text-2xl font-bold text-center mb-8 text-foreground reveal ${skillsVisible ? 'visible' : ''}`}
          >
            Skills & Expertise
          </h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {skillCategories.map((category, index) => (
              <div 
                key={category.title}
                className={`bg-gradient-card rounded-xl p-6 card-shadow border border-border hover:border-primary/30 hover:card-shadow-hover transition-all duration-300 group card-lift gradient-border reveal stagger-${index + 1} ${skillsVisible ? 'visible' : ''}`}
              >
                ...
                <div className="flex flex-wrap gap-2">
                  {category.skills.map((skill) => (
                    <Badge key={skill} variant="skill" className="hover:bg-primary/20 transition-colors cursor-default">
                      {skill}
                    </Badge>
                  ))}
                </div>
